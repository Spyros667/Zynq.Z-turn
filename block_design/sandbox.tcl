...
################################################################
# This is a generated script based on design: sandbox
#
...
# Hierarchical cell: Switches
proc create_hier_cell_Switches { parentCell nameHier } {

  variable script_folder

  if { $parentCell eq "" || $nameHier eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2092 -severity "ERROR" "create_hier_cell_Switches() - Empty argument(s)!"}
     return
  }
  ...
  create_bd_pin -dir O -from 0 -to 0 SW0
  create_bd_pin -dir O -from 0 -to 0 SW1
  create_bd_pin -dir O -from 0 -to 0 SW2
  create_bd_pin -dir O -from 0 -to 0 SW3
  ...
  connect_bd_net -net NOT_gate_Res  [get_bd_pins NOT_gate/Res] \
  [get_bd_pins xlslice_3/Din] \
  [get_bd_pins xlslice_2/Din] \
  [get_bd_pins xlslice_1/Din] \
  [get_bd_pins xlslice_0/Din]
  connect_bd_net -net SW_1  [get_bd_pins SW] \
  [get_bd_pins NOT_gate/Op1]
...
# Hierarchical cell: RGB_LED
proc create_hier_cell_RGB_LED { parentCell nameHier } {

  variable script_folder

  if { $parentCell eq "" || $nameHier eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2092 -severity "ERROR" "create_hier_cell_RGB_LED() - Empty argument(s)!"}
     return
  }
  ...
  # Create pins
  create_bd_pin -dir O -from 2 -to 0 RGB
  create_bd_pin -dir I -from 0 -to 0 Green
  create_bd_pin -dir I -from 0 -to 0 Red
  create_bd_pin -dir I -from 0 -to 0 Blue

  # Create instance: RGB_concat, and set properties
  set RGB_concat [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlconcat:2.1 RGB_concat ]
  set_property CONFIG.NUM_PORTS {3} $RGB_concat
  ...
  # Create port connections
  connect_bd_net -net In0_1  [get_bd_pins Green] \
  [get_bd_pins RGB_concat/In0]
  connect_bd_net -net In1_1  [get_bd_pins Blue] \
  [get_bd_pins RGB_concat/In1]
  connect_bd_net -net In2_1  [get_bd_pins Red] \
  [get_bd_pins RGB_concat/In2]
  connect_bd_net -net concatenation_dout  [get_bd_pins RGB_concat/dout] \
  [get_bd_pins NOT_gate/Op1]
  connect_bd_net -net util_vector_logic_0_Res  [get_bd_pins NOT_gate/Res] \
  [get_bd_pins RGB]
...
\
  ...
  # Create instance: RGB_LED
  create_hier_cell_RGB_LED [current_bd_instance .] RGB_LED

  # Create instance: Switches
  create_hier_cell_Switches [current_bd_instance .] Switches
  ...
  # Create port connections
  connect_bd_net -net Blue_1  [get_bd_pins Switches/SW2] \
  [get_bd_pins RGB_LED/Blue]
  connect_bd_net -net Green_1  [get_bd_pins Switches/SW1] \
  [get_bd_pins RGB_LED/Green]
  ...
  connect_bd_net -net RGB_LED_LEDS  [get_bd_pins RGB_LED/RGB] \
  [get_bd_ports RGB]
  connect_bd_net -net Red_1  [get_bd_pins Switches/SW0] \
  [get_bd_pins RGB_LED/Red]
  connect_bd_net -net SW_1  [get_bd_ports SW] \
  [get_bd_pins Switches/SW]
  connect_bd_net -net Switches_SW3  [get_bd_pins Switches/SW3] \
  [get_bd_ports BP]
